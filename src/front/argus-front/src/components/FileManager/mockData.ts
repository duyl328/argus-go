...
export const mockFolderStructure: FolderStructure = {
  Home: {
    ...
    type: 'folder',
    children: (() => {
      const homeItems: Record<string, any> = {
        'My Photos': {
          name: 'My Photos',
          type: 'folder',
          children: {
      ...
      // 在Home文件夹添加更多文件夹和文件，确保有足够内容触发滚动
      for (let i = 1; i <= 50; i++) {
        homeItems[`Folder_${String(i).padStart(2, '0')}`] = {
          name: `Folder_${String(i).padStart(2, '0')}`,
          type: 'folder',
          children: {}
        }
      }
      ...
      for (let i = 1; i <= 50; i++) {
        ...
        homeItems[`File_${String(i).padStart(2, '0')}.jpg`] = {
          name: `File_${String(i).padStart(2, '0')}.jpg`,
          type: 'photo',
          size: `${size} MB`,
